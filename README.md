# SkillTa — AI-Powered Tech Career Guidance Platform 🚀

> **Your all-in-one AI career copilot for the global tech industry.**
> Free tools for career discovery, roadmaps, salary intelligence, resume review, skill-gap analysis, and country-specific interview preparation guides — trusted by aspiring engineers, developers, and tech professionals worldwide.

🌐 **Live Website:** [https://www.skillta.tech](https://www.skillta.tech)
🐦 **X (Twitter):** [@SkilltaTech](https://x.com/SkilltaTech)
📧 **Contact:** [adarshmishra70931@gmail.com](mailto:adarshmishra70931@gmail.com)

---

## 🌟 What is SkillTa?

**SkillTa** is a free, AI-driven career guidance platform built to solve one of the biggest problems in tech education today: **career confusion**. Whether you're a student, career switcher, or working professional, SkillTa gives you the tools, data, and personalized guidance to navigate the modern tech industry with clarity.

Built on **React, Vite, Tailwind CSS, Framer Motion, Firebase Auth, and Supabase**, SkillTa combines beautiful UI/UX with powerful AI (Google Gemini) and rich, hand-curated career data covering **50+ tech roles across 50+ countries**.

---

## 🎯 Core Features & Services

### 1. 🧭 [Diagnostic Career Quiz](https://www.skillta.tech/quiz)
A scientifically designed personality + aptitude quiz that maps your interests, skills, and working style to the **best-fit tech career** for you. Get a detailed report with role matches, learning paths, and next steps.

### 2. 🗺️ [Career Roadmaps Library](https://www.skillta.tech/roadmaps)
Step-by-step **phase-wise roadmaps** for 50+ trending tech careers — Software Engineer, Data Scientist, AI/ML Engineer, DevOps, Cybersecurity, Cloud Architect, Product Manager, and more. Each roadmap includes skills, tools, salary ranges, reality checks, and downloadable **high-quality PDF exports**.

### 3. 💰 [Salary Predictor](https://www.skillta.tech/salary-predictor)
Get **unlimited, data-backed salary predictions** based on your role, location, experience, and skills. Covers salaries across USA, UK, Canada, Australia, Germany, India, and 45+ other countries.

### 4. ⚔️ [Compare Careers](https://www.skillta.tech/compare-careers)
Side-by-side comparison of tech roles across **salary, growth, difficulty, work-life balance, remote friendliness, and demand** — unlimited comparisons, always free.

### 5. 📄 [AI Resume Reviewer](https://www.skillta.tech/resume-reviewer)
Upload your resume + target role + (optional) job description → get **AI-powered analysis** with strengths, weaknesses, ATS score, keyword suggestions, and a downloadable PDF report. Free tier: **3 reviews/day**.

### 6. 🧠 [Skill Gap Analyzer](https://www.skillta.tech/skill-gap-analyzer)
Compare your current skills against your target role's requirements. Get a **personalized gap analysis, learning priorities, and resource recommendations**. Algorithmic (no AI cost), lightning-fast. Free tier: **3 analyses/day**.

### 7. 🌍 [Global Country Ecosystem](https://www.skillta.tech/usa)
**50+ countries × 10 sections each** = **500+ localized career hubs** covering:
- Overview & Tech Scene
- In-Demand Roles
- Salary Guide
- Top Companies
- Visa & Immigration
- Learning Resources
- Interview Culture
- Resume Standards
- Networking Communities
- Job Boards

### 8. 📝 [Tech Career Blog](https://www.skillta.tech/blog)
**70+ SEO-optimized long-form articles** on trending tech topics, country-specific salary guides, career switching, upskilling, and 2026 tech trends. Filter by country/category.

### 9. 🤖 Saathi Chatbot
An in-app decision-tree assistant that guides users through SkillTa's features and answers common career questions.

---

## 🛠️ Tech Stack

| Layer | Technology |
|---|---|
| **Frontend** | React 18, Vite 5, TypeScript 5, Tailwind CSS v3, Framer Motion, shadcn/ui |
| **Backend** | Supabase (PostgreSQL, Edge Functions, RLS), Firebase Auth (Google Sign-In) |
| **AI** | Google Gemini 1.5 Flash Lite (Resume Reviewer) |
| **Email** | Resend (welcome, plan expiry, limit-reached notifications) |
| **Hosting** | Vercel (SPA) — Custom domain: `www.skillta.tech` |
| **Analytics & SEO** | Google Search Console, Yandex Webmaster, JSON-LD, react-helmet-async |
| **Design System** | Dark charcoal theme • Teal/purple gradients • Glassmorphism • Space Grotesk + JetBrains Mono |

---

## 🌐 Site Map — Key URLs

### Main Pages
- 🏠 [Home](https://www.skillta.tech/)
- 💰 [Pricing](https://www.skillta.tech/pricing)
- 👤 [Dashboard](https://www.skillta.tech/dashboard)
- 📩 [Contact the Founder](https://www.skillta.tech/contact)
- 📜 [Terms of Service](https://www.skillta.tech/terms)
- 🔒 [Privacy Policy](https://www.skillta.tech/privacy)

### Free Services
- [Career Quiz](https://www.skillta.tech/quiz) · [Roadmaps](https://www.skillta.tech/roadmaps) · [Salary Predictor](https://www.skillta.tech/salary-predictor) · [Compare Careers](https://www.skillta.tech/compare-careers) · [Resume Reviewer](https://www.skillta.tech/resume-reviewer) · [Skill Gap Analyzer](https://www.skillta.tech/skill-gap-analyzer)

### Popular Country Hubs
- 🇺🇸 [Tech Careers in USA](https://www.skillta.tech/usa) · 🇬🇧 [UK](https://www.skillta.tech/uk) · 🇨🇦 [Canada](https://www.skillta.tech/canada) · 🇦🇺 [Australia](https://www.skillta.tech/australia) · 🇩🇪 [Germany](https://www.skillta.tech/germany) · 🇮🇳 [India](https://www.skillta.tech/india) · 🇸🇬 [Singapore](https://www.skillta.tech/singapore) · 🇦🇪 [UAE](https://www.skillta.tech/uae)

Full sitemap: [https://www.skillta.tech/sitemap.xml](https://www.skillta.tech/sitemap.xml)

---

## 🎓 Who is SkillTa For?

- 🎓 **Students** exploring tech careers for the first time
- 💼 **Career Switchers** moving into tech from other industries
- 👨‍💻 **Junior Developers** planning their next 3–5 years
- 🌍 **International Job Seekers** researching global tech markets
- 📈 **Working Professionals** upskilling or preparing for interviews
- 🧑‍🏫 **Educators & Career Counselors** looking for structured career data

---

## 💡 Why SkillTa Stands Out

✅ **Free forever core tools** — no hidden paywalls on essential guidance<br>
✅ **Global-first** — 50+ countries with localized salary, visa, and job market data<br>
✅ **Data-driven, not fluff** — every roadmap, salary, and comparison backed by curated research<br>
✅ **AI where it matters** — Gemini-powered resume review with cost-safe limits<br>
✅ **Beautiful, fast, mobile-first UX** — glassmorphism, smooth animations, PWA-ready<br>
✅ **SEO-optimized** — 650+ indexed pages, JSON-LD schema, per-route canonical tags<br>
✅ **Privacy-respecting** — Firebase Auth (Google), no tracking spyware

---

## 📊 By the Numbers

- 🌍 **50+ countries** with dedicated career ecosystems
- 🗺️ **50+ tech career roadmaps** with PDF exports
- 📝 **70+ SEO-optimized blog articles**
- 🎯 **500+ localized country × topic pages**
- 🛠️ **7 core free services** for career growth
- 🌐 **650+ URLs** in the sitemap

---

## 🤝 Get Involved

- 💬 **Have feedback?** [Contact the founder directly](https://www.skillta.tech/contact)
- 🐦 **Follow on X:** [@SkilltaTech](https://x.com/SkilltaTech) for career tips, updates, and tech insights
- 📧 **Partnerships / Media:** [adarshmishra70931@gmail.com](mailto:adarshmishra70931@gmail.com)
- 🐛 **Report a bug:** Use the contact form on the website

---

## 👨‍💻 About the Founder

**Adarsh Jayprakash Mishra** — A final-year Computer Science student, founder of SkillTa, and an aspiring software tester passoniate about ai, web development and startup.

- 💼 [LinkedIn](https://www.linkedin.com/in/adarsh-jayprakash-mishra)
- 🐙 [GitHub](https://github.com/Code-By-Adarsh)
- 📧 [Email](mailto:adarshmishra70931@gmail.com)

---

## ⭐ Support SkillTa

If SkillTa helped you find clarity in your career journey, the best way to support us is:

1. ⭐ **Share the website** with friends, classmates, or your community
2. 🐦 **Follow us on X:** [@SkilltaTech](https://x.com/SkilltaTech)
3. 📝 **Write about us** on your blog, LinkedIn, or Dev.to
4. 💬 **Send feedback** through our [contact page](https://www.skillta.tech/contact)

> **Made with ❤️ in India, for tech talent everywhere.**
